...
function
j3d_model_make_normals(model)
{
    model.normals = new
Array(model.faces.length);

for (var i = 0; i < model.faces.length; i++)
{
    var
v1 = j3d_vector_subtract(model.vertices[0][model.faces[i].indices[1]], model.vertices[0][model.faces[i].indices[0]]);
var
v2 = j3d_vector_subtract(model.vertices[0][model.faces[i].indices[2]], model.vertices[0][model.faces[i].indices[0]]);
...
function
j3d_model_multiply(model, matrix, mprime)
{
if (mprime == null)
mprime = {vertices: []};
...
function
j3d_model_dehomogenize(model, mprime)
{
if (mprime == null)
    mprime = {vertices: []};
